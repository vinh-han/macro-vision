...
class DatasetPipeline:
    def __init__(
        ...
        # relative paths to the dataset dir
        package_dir = Path(__file__).parent
        self.images_dir = str(package_dir / images_dir)
        self.labels_dir = str(package_dir / labels_dir)
        self.output_dir = str(package_dir / output_dir)
        ...
        self.logger = setup_logger(__name__, "pipeline.log")
    ...
    def run_visualization(self, split: str = "train", num_samples: int = 5, save_output: bool = False) -> None:
        ...
        self.visualizer.visualize_dataset(
            ...
            output_dir=str(Path(self.output_dir) / "visualizations")
        )
    ...
    def run_refinement(self) -> None:
        ...
        yaml_path = self.refiner.create_yolo_dataset_yaml(
            output_path=str(Path(self.output_dir) / "dataset.yaml"),
        ...
        coco_path = self.refiner.export_to_coco(
            output_path=str(Path(self.output_dir) / "annotations.json"),
            class_names=self.ingredients
        )
